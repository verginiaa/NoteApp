...
const SingleNote = ({ navigation, route }) => {
  ...
  console.log("props");
  const beginTitile = route.params.item.title;
  const beginDescription = route.params.item.description;

  const [input, setInput] = useState(beginTitile);
  const [description, setDescription] = useState(beginDescription);
      ...
      >
        ...
        <View style={globalStyles.body}>
          <TouchableOpacity
            ...
            onPress={() => {
              navigation.goBack();
              route.params.upd(input, description, route.params.item.key);
            }}
          >
